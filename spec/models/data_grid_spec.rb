...
\t<=80\t11\t22\t33
EOS
          dg_from_import("Grid", gr)
        end
        before (:each) do
          Marty::Feature.create!(name: "string_f", data_type: "string")
          Marty::Feature.create!(name: "int_f", data_type: "integer")
          Marty::Feature.create!(name: "float_f", data_type: "float")
          Marty::Feature.create!(name: "class_f", data_type: "Gemini::State")
        end
        it "checks constraint syntax" do
          expect { do_grid("string","string_f","float_f","<int_f")
          }.to raise_error(/operator required for feature float_f/)
          expect { do_grid("string","string_f","error float_f","<int_f")
          }.to raise_error(/invalid grid modifier expression: error float_f/)
          expect { do_grid("string","<string_f","<float_f","<int_f")
          }.to raise_error(/operator not allowed for feature string_f/)
        end
        it "rejects unknown features" do
          expect { do_grid("string","string_f",">float_f","> abcdef")
          }.to raise_error(/feature not found: abcdef/)
        end
        it "rejects incompatible feature type 1" do
          expected = /feature float_f type \(float\) is incompatible \
with grid dim type \(string\)/
          expect { do_grid("string","float_f",">float_f",">int_f")
          }.to raise_error(expected)
        end
        it "rejects incompatible feature type 2" do
          expected = /feature string_f type \(string\) is incompatible \
with grid dim type \(int4range\)/
          expect { do_grid("Gemini::State","string_f",">float_f",">string_f")
          }.to raise_error(expected)
        end
        it "allows compatible feature type" do
          expect { do_grid("string","string_f",">int_f",">int_f")
          }.to_not raise_error
        end
        it "allows compatible feature type 2" do
          expect { do_grid("string","string_f",">float_f",">float_f")
          }.to_not raise_error
        end
        it "allows good constraints" do
          expect { do_grid("Gemini::State","class_f",">float_f",">int_f")
          }.to_not raise_error
        end
        it "rejects incompatible feature type 4" do
          expected = /feature class_f type \(Gemini::State\) is \
incompatible with grid dim type \(string\)/
          expect { do_grid("string","class_f",">float_f",">int_f")
          }.to raise_error(expected)
        end
      end
    end
