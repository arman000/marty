...
*/
  ...
  def check_grid(grid_name)
    ...
    # colors / data check in dim area
    check_dim = lambda do |dim, dim_size, dim_area, by_col: true|
      if dim.count > 0
        color_sets = Array.new(dim_size) { Set.new }
        iterate_area.call(dim_area, ui_colors) do |cell, row_idx, col_idx|
        ...
        dim_data = iterate_area.call(dim_area, ui_data)
      ...
      # vertical dims
    check_dim.call(vdim, vdim_size, vdim_area)
    check_dim.call(hdim, hdim_size, hdim_area, by_col: false)
    ...
    # data and label section data check
  ...
  it 'dg editor' do
    ...
    wait_for_ajax
    # FIXME weird behavior here.  save doesn't close window
    press('Save')
    wait_for_ajax
    grid = get_latest.call
    ...
    grid_meta = grid.metadata.map {|md| [md['dir'], md['attr'], md['keys']]}.sort
