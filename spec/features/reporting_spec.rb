...
feature 'under Applications menu, Reports workflows',
        js: true,
        download: true do
  before(:all) do
    marty_whodunnit
...
DELOREAN
  ...
  it 'AA node generates proper CSV via generate report' do
    ...
    wait_for_element do
      expect(download_content_acceptable?).to be_truthy
    end

    and_by 'validate download' do
      expect(download_content).to have_content('XYZ1,XYZ2,XYZ3,XYZ4 1,2,3,4 ' +
  ...
  it 'report screen handles security roles properly - enabled' do
    ...
    and_by 'correct button shown' do
      fill_in('Note Rate', with: '3.00')
  ...
  it 'file upload field works on bogus report' do
    ...
    wait_for_element do
      download_content_acceptable?
    end

    and_by 'validate download' do
      expect(download_content).to have_content('NodeC:')
    end
  end
end
