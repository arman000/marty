...
class Marty::Aws::Apigateway
  ...
  def get_rest_apis
    ...
    # filter rest apis by config or marty
    api_name = (Marty::Config['AWS_APP_IDENTIFIER'] || 'marty')

    # ensure that api is associated with deployment based on environment
    resp.items = resp.items.select do |i|
      i.name.include?(api_name) &&
        get_stages(i.id).detect{|s| s.stage_name == Rails.env}
    end
  ...
  def get_usage usage_plan_id, key_id
    # we are limited to a 90 day window
    today = Date.today
    client.get_usage(
      usage_plan_id: usage_plan_id,
      key_id:        key_id,
      start_date:    (today - 90.days).to_s,
      end_date:      today.to_s,
      limit:         1,
    )
  end

  def get_usage_plans api_id=nil
    return [] unless api_id

    client.get_usage_plans(limit: 500).items.select do |p|
      p.api_stages.any?{|s| s.api_id == api_id && s.stage == Rails.env}
    end
  end
  ...
  def create_usage_plan name, description, api_id, throttle, quota
    client.create_usage_plan(
      name: name,
      description: description,
      api_stages: [{'api_id' => api_id, 'stage' => Rails.env}],
      throttle: throttle,
      quota: quota,
    )
  end
  ...
  def delete_usage_plan api_id, usage_plan_id

    raise 'Cannot delete a usage plan that is associated '\
          'with API keys. Delete keys or move keys to a new usage plan.' unless
      get_usage_plan_keys(usage_plan_id).empty?

    # remove staging association from usage plan
  ...
  def swagger_export api_id
    client.get_export(
      rest_api_id: api_id,
      stage_name: Rails.env,
      export_type: 'swagger',
  ...
  def swagger_import swagger_body
    client.import_rest_api(
      fail_on_warnings: true,
      parameters: {base_path: 'ignore'},
      body: swagger_body,
    )
  end

  def swagger_update api_id, swagger_body, mode='merge'
    client.put_rest_api(
      rest_api_id: api_id,
      mode: mode,
      body: swagger_body,
  ...
  def create_deployment api_id, desc
